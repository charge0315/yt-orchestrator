...
router.get('/me', authenticate, async (req: AuthRequest, res: Response) => {
  ...
  try {
    if (!req.session) {
      return res.status(401).json({ error: 'セッションがありません' });
    }
  ...
  try {
    ...
    if (mongoose.connection.readyState === 1) {
      try {
        ...
        console.log('✅ ユーザートークンを MongoDB に保存しました');
      } catch (dbError) {
        console.error('ユーザー情報の MongoDB 保存に失敗しました:', dbError);
    ...
    console.error('Google OAuth コールバックエラー:', error);
